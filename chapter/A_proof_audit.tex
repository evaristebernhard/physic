\section{Proof audit and technical checkpoints}
...
The finite--$Z$ theorem is not obtained by replacing a printed decimal inside the final HPS statement.  What is used is the symbolic cubic-weight chain in \cite[Sec.~7.3]{HPS2025}.  In particular, their finite--configuration estimate has the form
\[
...
\]
so the relevant interval is $[\beta_3^{-1},9/4]$.  Interval evaluation of the explicit closed expressions gives
\[
 F_{\beta_3}(\beta_3^{-1})
 =3.81147042506\ldots,
 \qquad
...
All decimals in \Cref{thm:finite-Z} are chosen strictly on the safe side of explicit closed expressions.  The repository script \texttt{verify\_constants.py} evaluates the formulas with interval arithmetic, so the inequalities below do not rely on ordinary floating-point rounding:
\[
...
\]
These are evaluations of elementary explicit formulas involving $\exp$, $\arctan$, radicals and $\pi$; no numerical optimization enters the theorem.  The interval script is only a reproducibility check for the displayed decimal margins; the analytic proof is contained in the preceding sections.
