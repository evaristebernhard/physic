...
The excess--charge problem asks how many electrons an atomic nucleus of charge $Z$ can bind.  In the nonrelativistic many--electron Schr\"odinger model, the ionization conjecture predicts that only a uniformly bounded number of electrons can be bound in excess of neutrality,
\[
 N_c(Z)\le Z+C
\]
with a universal constant $C$.  This remains open for the full Schr\"odinger problem; see the reviews \cite{Nam2013Review,Nam2022Review}.  The conjecture is known in important approximation models, including Hartree--Fock theory \cite{Solovej2003}, but not for the exact Coulomb Hamiltonian.

The rigorous history has both asymptotic and finite--$Z$ strands.  Early work of Ruskai and Sigal established nonexistence of bound states for sufficiently highly charged negative ions \cite{Ruskai1982,Sigal1982}.  Lieb, Sigal, Simon and Thirring later proved asymptotic neutrality, $N_c(Z)/Z\to1$ as $Z\to\infty$ \cite{LSST1988}; quantitative large--$Z$ refinements were obtained by Fefferman--Seco and by Seco--Sigal--Solovej \cite{FeffermanSeco1990,SecoSigalSolovej1990}.  These asymptotic results do not by themselves give a sharp uniform finite--$Z$ estimate.

For arbitrary finite $Z$, Lieb proved the classical non--asymptotic bound $N_c(Z)<2Z+1$ \cite{Lieb1984}.  Nam later improved this to
\[
 N_c(Z)<1.22\,Z+3Z^{1/3},
\]
for fermionic atoms \cite{Nam2012}.

A recent refinement by Hundertmark, Pattakos and Schulz (HPS) develops this weighted multiplier strategy substantially further and proves
\[
...
The paper is organized as follows.  \Cref{sec:radial} records the radial reduction.  \Cref{sec:log} introduces the logarithmic variational problem.  \Cref{sec:extremizer} constructs the candidate maximizer and proves its obstacle condition.  \Cref{sec:global} proves global optimality for the radial problem.  \Cref{sec:atomic} propagates the exact constant through the HPS finite--$N$ argument and proves \Cref{thm:finite-Z}.  \Cref{sec:outlook} discusses the significance and limitations of the result and the information required for improvements beyond the cubic mean--field obstruction.  The appendix supplies supplementary details for the functional--analytic and finite--$Z$ steps used in the main argument.
