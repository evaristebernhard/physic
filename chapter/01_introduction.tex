...
\]
Up to the scaling invariance of \eqref{eq:def-beta-s}, this minimizer is unique.
\end{theorem}
...
\[
 k(t)=\ee^{-|t|}-\ee^{-2|t|}.
\]
Second, the Euler--Lagrange obstacle condition determines a compactly supported cosine profile.  Third, a conditional negative--definiteness estimate on the extremal support rules out all competing maximizers.  The last step is where the exact solution becomes global rather than merely stationary.
...
The paper is organized as follows.  \Cref{sec:radial} records the radial reduction.  \Cref{sec:log} introduces the logarithmic variational problem.  \Cref{sec:extremizer} constructs the candidate maximizer and proves its obstacle condition.  \Cref{sec:global} proves global optimality.  \Cref{sec:atomic} explains how the improved value of $\beta_3$ enters the HPS excess--charge framework.  The appendix isolates the technical points that are most sensitive in a formal proof audit.
